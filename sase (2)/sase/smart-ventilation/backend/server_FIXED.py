#!/usr/bin/env python3
"""
AI-Powered Smart Window Ventilation & Environmental Analytics System - Flask Backend Server (FIXED)
...
# 🔴 FIXED: Separate manual override with persistence + timestamp
manual_override = {
    "angle": None,
    "timestamp": None,
    "duration_ms": 120000  # 120 seconds - manual override persists for 2 minutes
}
...
# ─────────────────────────────────────────────
# HELPER: Check if manual override is still valid
# ─────────────────────────────────────────────
def is_manual_override_active():
    """Returns angle if override is still active, None otherwise"""
    with lock:
    ...
    while True:
        with lock:
            if sensor_state["source"] == "simulation":
                ...
                # 🔴 FIXED: Check if manual override is active
                manual_angle = is_manual_override_active()
                current_angle = manual_angle if manual_angle is not None else decision["recommended_angle"]
...
def receive_sensor_data():
    """ESP32 posts sensor readings here and receives servo command"""
    global sensor_state
...
def manual_control():
    """
    Set manual window angle. 🔴 FIXED: Now persists command for 120 seconds
    Multiple calls can refresh the duration.
    """
    ...
    with lock:
        ...
        manual_state = dict(manual_override)
...
def servo_proxy():
    ...
    This endpoint ALSO sets manual override to ensure servo moves.
    """
    data = request.get_json()
    if not data or "angle" not in data:
        return jsonify({"error": "angle field required"}), 400
...
if __name__ == '__main__':
    ...
    print("  VERSION: 1.0.1 (MANUAL OVERRIDE FIX)")
    print("=" * 60)
    print(f"  Dashboard:     http://localhost:5000")
    print(f"  API Data:      http://localhost:5000/api/data")
    print(f"  ESP32 POST:    http://localhost:5000/api/sensor")
    print(f"  Manual Ctrl:   http://localhost:5000/api/manual-control (120s persistence)")
    print(f"  Servo Proxy:   http://localhost:5000/api/servo (new endpoint)")
    print("=" * 60)
